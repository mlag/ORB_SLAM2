...
std::vector<std::pair<double, cv::Mat>> System::GetTrajectory() const {
    std::vector<std::pair<double, cv::Mat>> result;
    vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
        ...
        lit != lend; lit++, lRit++, lT++)
    {
        ...
        Rwc.copyTo(pose(cv::Range(0,3), cv::Range(0,3)));
        twc.copyTo(pose(cv::Range(0,3), cv::Range(3,4)));

        result.emplace_back(*lT, std::move(pose));
    }
    return result;
}
